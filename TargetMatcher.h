...
class TargetMatcher {
  ...
  private:
	  ...
	  cv::Mat target;      //目标图像
	  int normType;        //比较描述子的方式
	  double distance;     //最小重投影误差
	  int numberOfLevels;  //金字塔图像的数量
	  double scaleFactor;  //层级之间的scale factor
	  //目标图像构建的金字塔以及它们的关键点
	  ...
	  void createPyramid() {
		  ...
		  for (int i = 0; i < numberOfLevels; i++) { // reduce size at each layer
			  ...
			  cv::resize(target, target, cv::Size(), scaleFactor, scaleFactor);
		  }
